...
var require_base = __commonJS({
  "node_modules/asn1.js/lib/asn1/base/index.js"(exports2) {
    ...
    base2.DecoderBuffer = require_buffer().DecoderBuffer;
    base2.EncoderBuffer = require_buffer().EncoderBuffer;
...
async function deleteItem(key) {
  await doc.send(new import_lib_dynamodb.DeleteCommand({ TableName: TABLE, Key: key }));
}
...
async function queryGsi1(gsi1pk) {
  const out = await doc.send(
    new import_lib_dynamodb.QueryCommand({
      TableName: TABLE,
      IndexName: "GSI1",
...
async function scanByType(type, extra) {
  const out = await doc.send(
...
// functions/src/lib/leaderboard.ts
function aggregateLeaderboard(responses, names) {
  ...
  return [...agg.entries()].map(([participantId, v]) => ({
    participantId,
...
var smc = new import_client_secrets_manager.SecretsManagerClient({});
...
async function ensureVapid() {
  ...
  if (!arn) return false;
  try {
    const out = await smc.send(new import_client_secrets_manager.GetSecretValueCommand({ SecretId: arn }));
    const v = JSON.parse(out.SecretString || "{}");
    if (!v.publicKey || !v.privateKey) return false;
    import_web_push.default.setVapidDetails(v.subject || "mailto:admin@roamthrough.com", v.publicKey, v.privateKey);
    vapidReady = true;
    return true;
  } catch {
    return false;
...
async function sendTestPush(uid) {
  if (!await ensureVapid()) return false;
  const subs = await queryPartition(`USER#${uid}`, "PUSHSUB#");
  if (!subs.length) return false;
  const payload = JSON.stringify({ title: "\u{1F33F} Tequila Roam", body: "Push notifications are working \u2014 salud!", url: "/home" });
  let ok = false;
  for (const s of subs) {
    try {
      await import_web_push.default.sendNotification({ endpoint: s.endpoint, keys: { p256dh: s.p256dh, auth: s.auth } }, payload);
      ok = true;
    } catch (e) {
...
var handler = async (event) => {
  ...
  switch (fieldName) {
    case "listBottles":
      return stripKeys(await queryGsi1("BOTTLE"));
    case "bottlePopularity": {
      const favs = await scanByType("Favorite");
    ...
    case "getBottle":
      return stripOne(await getItem(keys.bottle(a.id)));
    case "getScanResult": {
      const ptr = await getItem(keys.scan(a.imageKey));
      if (!ptr?.bottleId) return null;
      return stripOne(await getItem(keys.bottle(ptr.bottleId)));
    }
    case "listFlights":
      return queryGsi1("FLIGHT");
    case "getSession":
      return stripOne(await getItem(keys.session(a.sessionId)));
    case "listParticipants":
      return stripKeys(await queryPartition(`SESSION#${a.sessionId}`, "PART#"));
    case "listRatings":
      return stripKeys(
        await queryPartition(`SESSION#${a.sessionId}`, `RATING#${a.bottleId}#`)
      );
    case "listReviews": {
      const all = await queryPartition(`BOTTLE#${a.bottleId}`, "REVIEW#");
      return stripKeys(
    ...
    case "listMyHistory": {
      const rows = stripKeys(await queryPartition(`USER#${requireUser(event)}`, "HIST#"));
      return rows.sort((x, y) => (y.createdAt || "").localeCompare(x.createdAt || ""));
    }
    case "listMyFavorites":
      return stripKeys(await queryPartition(`USER#${requireUser(event)}`, "FAV#")).map((f) => f.bottleId);
    case "listMyShelf":
      return stripKeys(await queryPartition(`USER#${requireUser(event)}`, "SHELF#")).map((s) => ({ bottleId: s.bottleId, status: s.status ?? "owned", addedAt: s.addedAt ?? "" }));
    case "listMyReminders": {
      const rows = stripKeys(await queryPartition(`USER#${requireUser(event)}`, "REMIND#"));
    ...
    case "adminAiUsage": {
      ...
      const rows = (await queryPartition("AICOST")).sort((x, y) => (y.SK || "").localeCompare(x.SK || "")).slice(0, days);
      const PRICE = {
        opus: { in: 15, out: 75 },
        sonnet: { in: 3, out: 15 },
        haiku: { in: 1, out: 5 }
      ...
      return Object.values(agg).map((x) => {
        ...
        const cost = x.inputTokens / 1e6 * p.in + x.outputTokens / 1e6 * p.out + x.cacheReadTokens / 1e6 * p.in * 0.1 + x.cacheWriteTokens / 1e6 * p.in * 1.25;
        return { ...x, cost: Math.round(cost * 1e4) / 1e4 };
      }).sort((m, n) => n.cost - m.cost);
    }
    case "adminAlarms": {
      if (!isAdmin(event)) throw new Error("admins only");
      const out = await cw.send(
        new import_client_cloudwatch.DescribeAlarmsCommand({ AlarmTypes: ["MetricAlarm"], AlarmNamePrefix: "agave-", MaxRecords: 100 })
      );
      return (out.MetricAlarms ?? []).map((al) => ({
        name: al.AlarmName ?? "",
        ...
        const rank = (s) => s === "ALARM" ? 0 : s === "INSUFFICIENT_DATA" ? 1 : 2;
...
async function submitFeedback(a) {
  const message = String(a.message ?? "").trim().slice(0, 4e3);
  if (!message) throw new Error("message required");
  const at = (/* @__PURE__ */ new Date()).toISOString();
  await putItem({
    PK: "FEEDBACK",
    SK: `${at}#${(0, import_node_crypto.randomUUID)()}`,
    type: "Feedback",
...
async function startSession(a, event) {
  const sessionId = (0, import_node_crypto.randomUUID)();
  const joinCode = (0, import_node_crypto.randomUUID)().slice(0, 4).toUpperCase();
...
async function advanceSession(a) {
  const existing = await getItem(keys.session(a.sessionId));
...
async function joinSession(a) {
  const [found] = await queryGsi1(`JOIN#${a.joinCode}`);
  if (!found) throw new Error("invalid join code");
  const participant = {
    sessionId: found.sessionId,
    participantId: (0, import_node_crypto.randomUUID)(),
...
async function submitRating(input) {
  const rating = { ...input, syncedAt: (/* @__PURE__ */ new Date()).toISOString() };
  await putItem({
...
async function track(name, path) {
...
async function adminPageViews() {
  const rows = await queryPartition("PAGEVIEWS", "");
  return rows.map((r) => ({ page: r.SK, count: r.count ?? 0 })).sort((a, b) => b.count - a.count);
}
async function adminAnalytics(days) {
  const rows = await queryPartition("ANALYTICS", "");
  return rows.map((r) => ({
    day: r.day ?? r.SK,
  ...
  })).sort((a, b) => b.day.localeCompare(a.day)).slice(0, Math.max(1, Math.min(days, 90)));
}
async function upsertReview(input, event) {
  const userId = event.identity?.sub;
  if (!userId) throw new Error("auth required");
  const review = {
    bottleId: input.bottleId,
    userId,
    displayName: event.identity?.claims?.["name"] ?? "Member",
    body: input.body,
    score: input.score,
    aroma: input.aroma,
    palate: input.palate,
    ...
    moderation: input.published ? "pending" : void 0,
    createdAt: (/* @__PURE__ */ new Date()).toISOString()
  };
  await putItem({ ...keys.review(input.bottleId, userId), ...review, type: "Review" });
  return review;
}
async function recognizeBottle(imageKey) {
  const arn = process.env.PIPELINE_ARN;
  if (!arn) throw new Error("PIPELINE_ARN not set");
  await sfn.send(
    new import_client_sfn.StartExecutionCommand({ stateMachineArn: arn, input: JSON.stringify({ imageKey }) })
  );
  return null;
}
async function presignUpload(bottleId, contentType) {
  const bucket = process.env.UPLOAD_BUCKET;
  const ext = (contentType.split("/")[1] || "jpg").replace("jpeg", "jpg");
  const key = `media/${slugId(bottleId)}/${(0, import_node_crypto.randomUUID)()}.${ext}`;
  const uploadUrl = await (0, import_s3_request_presigner.getSignedUrl)(
    s3,
    new import_client_s3.PutObjectCommand({ Bucket: bucket, Key: key, ContentType: contentType }),
    { expiresIn: 300 }
  );
  const origin = process.env.APP_ORIGIN || "";
  return { uploadUrl, key, publicUrl: `${origin}/${key}` };
}
async function setBottleImage(bottleId, imageUrl) {
  const b = await getItem(keys.bottle(bottleId));
  if (!b) throw new Error("bottle not found");
  const updated = { ...b, imageUrl, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
  await putItem({ ...keys.bottle(bottleId), ...updated, type: "Bottle", gsi1pk: "BOTTLE", gsi1sk: b.name });
  return stripOne(updated);
}
function slugId(s) {
  return s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60) || "misc";
}
async function setReviewModeration(bottleId, userId, decision) {
  const r = await getItem(keys.review(bottleId, userId));
  if (!r) throw new Error("review not found");
  const approve = decision === "approve";
  const updated = { ...r, moderation: approve ? "approved" : "blocked", published: approve };
...
async function adminPatchBottle(id, patch) {
  const b = await getItem(keys.bottle(id));
  if (!b) throw new Error("bottle not found");
  const next = { ...b };
  ...
  if (typeof patch.abv === "number") {
    next.abv = patch.abv;
    next.proof = Math.round(patch.abv * 2);
  }
  if (patch.expression && ACCENTS[patch.expression]) {
    next.expression = patch.expression;
    next.accent = ACCENTS[patch.expression];
  }
  next.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  await putItem({ ...keys.bottle(id), ...next, type: "Bottle", gsi1pk: "BOTTLE", gsi1sk: next.name });
  return stripOne(next);
}
...
async function saveMyFlight(uid, input) {
  const flight = { id: input.id, title: input.title, subtitle: input.subtitle, bottleIds: input.bottleIds, curated: false };
  await putItem({ ...keys.userFlight(uid, input.id), ...flight, type: "UserFlight" });
  return flight;
}
async function deleteMyFlight(uid, id) {
  await deleteItem(keys.userFlight(uid, id));
  return true;
}
async function saveMyTasting(uid, input) {
  const entry = {
    id: input.id,
...
async function setFavorite(uid, bottleId, on) {
...
function endpointHash(endpoint) {
  return (0, import_node_crypto.createHash)("sha256").update(endpoint).digest("hex").slice(0, 24);
}
async function savePushSub(uid, subscription) {
...
async function removePushSub(uid, endpoint) {
  if (!endpoint) return false;
  await deleteItem(keys.pushSub(uid, endpointHash(endpoint)));
  return true;
}
async function setShelf(uid, bottleId, status) {
  const valid = ["owned", "wishlist", "tasted"];
  if (status && valid.includes(status)) {
    await putItem({ ...keys.shelf(uid, bottleId), bottleId, status, type: "Shelf", addedAt: (/* @__PURE__ */ new Date()).toISOString() });
    return true;
  }
  await deleteItem(keys.shelf(uid, bottleId));
  return false;
}
async function saveMyReminder(uid, input) {
  const r = {
    id: input.id,
    title: input.title,
...
async function leaderboard(sessionId) {
  const responses = await queryPartition(`SESSION#${sessionId}`, "QRESP#");
  const parts = await queryPartition(`SESSION#${sessionId}`, "PART#");
...
function stripKeys(items) {
  return items.map(stripOne).filter(Boolean);
}
function stripOne(item) {
