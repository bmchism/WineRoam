...
var reminderSweep_exports = {};
__export(reminderSweep_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(reminderSweep_exports);
...
// functions/src/lib/reminders.ts
function dueReminders(all, nowIso) {
  return all.filter((r) => r.when <= nowIso);
}

// functions/src/reminderSweep.ts
var sm2 = new import_client_secrets_manager2.SecretsManagerClient({});
...
async function ensureVapid() {
  ...
  if (!arn) return false;
  try {
    const out = await sm2.send(new import_client_secrets_manager2.GetSecretValueCommand({ SecretId: arn }));
...
async function sendPush(userId, title, body, url) {
  const subs = await queryPartition(`USER#${userId}`, "PUSHSUB#");
...
var APP_ORIGIN = process.env.APP_ORIGIN || "https://wine.roamthrough.com";
var handler = async () => {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const all = await queryGsi1("REMINDER");
...
${APP_ORIGIN}/tastings`
    ...
    if (wantSms && r.phone) {
      jobs.push(sendSms({ to: r.phone, body: `\u{1F33F} ${r.title}: ${r.message} \u2014 ${APP_ORIGIN}/tastings` }));
    }
    let delivered = false;
    if (jobs.length) {
