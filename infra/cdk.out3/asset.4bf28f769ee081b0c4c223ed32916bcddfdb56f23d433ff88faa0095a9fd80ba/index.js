...
var moderate_exports = {};
__export(moderate_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(moderate_exports);
...
// functions/src/moderate.ts
var SYSTEM = `You moderate community reviews for a wine tasting app.
...
var handler = async (event) => {
  ...
  const review = await getItem(keys.review(bottleId, userId));
  ...
  try {
    ...
    const j2 = extractJson(out);
    const verdict = j2?.verdict === "flag" ? "flag" : "ok";
    return { verdict, reason: j2?.reason || (verdict === "flag" ? "Flagged by AI." : "Looks clean.") };
  } catch {
