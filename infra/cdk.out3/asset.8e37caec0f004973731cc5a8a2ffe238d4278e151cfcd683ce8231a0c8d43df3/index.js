...
async function chat(system, messages, tier = "sonnet", maxTokens = 600) {
...
function normalize(s2) {
  return s2.toLowerCase().trim().replace(/\s+/g, " ").replace(/[?.!]+$/, "");
}
function hash(s2) {
  let h2 = 5381;
  for (let i2 = 0; i2 < s2.length; i2++) h2 = (h2 << 5) + h2 + s2.charCodeAt(i2) >>> 0;
  return h2.toString(16);
}
var cacheKey = (msg) => ({ PK: `CHAT#${hash(normalize(msg))}`, SK: "#R" });
var SYSTEM = `You are Wine Roam's in-app wine guide \u2014 warm, knowledgeable, and concise.
Answer questions about wine and wines: how it's made, brands, NOMs, expressions
(blanco, reposado, a\xF1ejo, extra a\xF1ejo, cristalino, high-proof), additive-free vs additives,
...
var handler = async (event) => {
  const message = (event.arguments.message ?? "").slice(0, 2e3).trim();
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      history = parsed.filter((t2) => t2 && (t2.role === "user" || t2.role === "assistant") && typeof t2.content === "string").slice(-8).map((t2) => ({ role: t2.role, content: String(t2.content).slice(0, 2e3) }));
    }
  } catch {
  }
  const cacheable = history.length === 0;
  if (cacheable) {
    const hit = await getItem(cacheKey(message));
  ...
  try {
    ...
    if (cacheable && reply) {
      await putItem({ ...cacheKey(message), reply: out, type: "ChatCache", createdAt: (/* @__PURE__ */ new Date()).toISOString() });
    }
    return out;
  } catch {
