...
var quizGenerate_exports = {};
__export(quizGenerate_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(quizGenerate_exports);
...
// functions/src/quizGenerate.ts
var SYSTEM = `You write fun, fair multiple-choice tasting-quiz questions about specific wines.
...
var handler = async (event) => {
  ...
  for (const id of event.bottleIds) {
    const b2 = await getItem(keys.bottle(id));
    if (b2) bottles.push(b2);
  }
  const facts = bottles.map(
    (b2) => `- ${b2.name}: ${b2.expression}, NOM ${b2.nom}, ${b2.abv}%, ${b2.grapeRegion}` + (b2.crushing ? `, crush: ${b2.crushing}` : "") + (b2.aging ? `, aging: ${b2.aging}` : "") + (b2.additiveFree ? ", additive-free" : "")
  ).join("\n");
...
Return the quiz JSON.`, maxTokens: 1400 },
    (t2) => {
      const j2 = extractJson(t2);
      return Boolean(j2?.questions && j2.questions.length >= 3);
    },
    ["haiku", "sonnet"]
  );
  const parsed = extractJson(text);
  const quizId = (0, import_node_crypto.randomUUID)();
  const questions = (parsed?.questions ?? []).slice(0, 5).map((q2) => ({
    quizId,
    questionId: (0, import_node_crypto.randomUUID)(),
    text: q2.text,
    options: q2.options,
  ...
  const quiz = { quizId, tastingId: event.tastingId, questions };
  await putItem({
    PK: `QUIZ#${quizId}`,
    SK: "#META",
    ...quiz,
