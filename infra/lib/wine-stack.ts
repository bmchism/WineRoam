...
interface WineStackProps extends StackProps {
  config: WineConfig;
...
export class WineStack extends Stack {
  constructor(scope: Construct, id: string, props: WineStackProps) {
    ...
    // ---------------------------------------------------------------- Data
    // NOTE: The physical DynamoDB table is WineStack-TableCD117FA1-1ULUK9Q4FALU7.
    // CloudFormation's state has a stale reference to a deleted physical ID.
    // We import the table by attributes to bypass the CFN drift. Using
    // fromTableAttributes with grantIndexPermissions ensures IAM grants cover
    // the GSI1 index as well.
    const table = dynamodb.Table.fromTableAttributes(this, "Table", {
      tableArn: "arn:aws:dynamodb:us-east-1:012185415876:table/WineStack-TableCD117FA1-1ULUK9Q4FALU7",
      grantIndexPermissions: true,
    });

    // -------------------------------------------------------------- Secrets
    const anthropicSecret = new secrets.Secret(this, "AnthropicKey", {
      description: "Wine Roam — Anthropic Workspace API key (set value after deploy)",
    });
    const anthropicAdminSecret = new secrets.Secret(this, "AnthropicAdminKey", {
      description: "Wine Roam — Anthropic Admin API key for org cost reporting",
    });
    const vapidSecret = new secrets.Secret(this, "VapidKey", {
      description: "Wine Roam — Web Push VAPID {publicKey, privateKey, subject} (set value after deploy)",
    });
    // Provider creds — swappable by updating the secret value (no redeploy).
    const resendSecret = new secrets.Secret(this, "ResendKey", {
      description: "Wine Roam — Resend {apiKey, fromEmail}. Shareable now; swap value to switch accounts.",
    });
    const twilioSecret = new secrets.Secret(this, "TwilioKey", {
      description: "Wine Roam — Twilio {accountSid, authToken, fromNumber}. Swap value to switch accounts.",
    });
    ...
    if (config.resendCognitoEmails) {
      emailKey = new kms.Key(this, "CognitoEmailKey", {
        description: "Wine Roam Cognito custom email sender",
    ...
    // Only add the trigger if we own the pool (legacy mode). In shared-auth mode,
    // the trigger is added in SpiritAuthStack or via a separate mechanism.
    ...
    const apiWaf = new wafv2.CfnWebACL(this, "ApiWaf", {
      scope: "REGIONAL",
      defaultAction: { allow: {} },
      visibilityConfig: { cloudWatchMetricsEnabled: true, metricName: "wineApi", sampledRequestsEnabled: true },
      rules: [
        {
          name: "ApiRateLimit",
          priority: 1,
    ...
    // -------------------------------------------------------- Monitoring
    const alarmTopic = new sns.Topic(this, "Alarms", { displayName: "Wine Roam alarms" });

    // App alarms use an "wine-" name prefix so the admin dashboard can list only
    ...
    errAlarm("ApiErrors", "wine-api-errors", apiFn, 5, 5);
    errAlarm("EnrichErrors", "wine-enrich-errors", enrichFn, 3, 15);
    errAlarm("ReminderErrors", "wine-reminder-errors", reminderFn, 2, 15);
    errAlarm("ChatErrors", "wine-chat-errors", chatFn, 5, 15);

    new cw.Alarm(this, "DynamoThrottles", {
      alarmName: "wine-dynamodb-throttles",
